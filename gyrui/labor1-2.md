Remek taktika! Valóban felesleges elveszni a laborok hosszú leírásaiban, ha a vizsgán célzottan ezekre a fogalmakra lőnek. Nézzük is az 1. és 2. heti laborok sűrítményét, kifejezetten a megadott ZH-kérdésekre fókuszálva.
...
Amikor egy lefordított C program elindul Linux alatt, az operációs rendszer egy jól meghatározott memóriastruktúrát (folyamat virtuális gép) hoz létre számára. A program a memóriában alapvetően az alábbi logikai részekre (szegmensekre) oszlik:
...
A Linux (és minden UNIX-szerű rendszer) alapelve, hogy minden futó program kap alapból 3 darab megnyitott adatcsatornát (fájlleírót):
...
* **Általános feladata:** Lehetővé teszi, hogy a program működését, beállításait (pl. melyik I2C címen keressen eszközt, vagy milyen részletes legyen a logolás) a kód újrafordítása nélkül tudjuk módosítani.
* Ennek leggyorsabb módja a program indításakor megadott **parancssori paraméterek** használata.

### A `main()` függvény prototípusai
A C program belépési pontja a `main()` függvény. Ahhoz, hogy a Linux át tudja adni a parancssori paramétereket a programunknak, a `main`-t különböző prototípusokkal lehet definiálni. Csak a jellegüket kell tudni:
...
A parancssori paraméterek jellemzően opciókból (kapcsolókból) és argumentumokból állnak (pl. `-h` helphez, vagy `-f config.txt` fájl megadásához).
...
* **Célja:** Szabványos, robusztus módon "kiparcolja" (feldolgozza) a rövid (egybetűs, mínusz jellel kezdődő) parancssori kapcsolókat. Nem kell tudnod a pontos paraméterezését, csak azt, hogy **ez a beépített eszköz a parancssori argumentumok kulturált feldolgozására**.

## 4. Az `strace` célja és működése
Ez egy igazi "jolly joker" eszköz Linux alatt a hibakereséshez.
